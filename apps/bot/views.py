...
class GithubView(CSRFExemptMixin, View):
    ...
    @staticmethod
    def _get_image_urls_from_text(
        comment: str,
    ) -> tuple[list[PhotoAttachment] | None, str]:
        img_re = r"<img\b[^>]*\bsrc\s*=\s*[\"']([^\"']+)[\"'][^>]*\/?>"
        image_urls = re.findall(img_re, comment, flags=re.I)
        if not image_urls:
            return None, comment
        photo_attachments = []
    ...
    def post(self, request):
        ...
        if data["action"] == "closed":
            ...
            data["action"] == "created"
            and data.get("comment")
            and data["comment"]["user"]["id"] == data["issue"]["user"]["id"]
        ):
