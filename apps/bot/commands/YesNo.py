...
class YesNo(CommonCommand):
    ...
    def start(self):
        clear_msg = replace_similar_letters(self.event.clear_msg)
        if clear_msg.lower() in ['идиот?', 'ты идиот?']:
            return "Мне потанцевать нельзя?"
        elif clear_msg.lower() in ['да?']:
            return {'attachments': 'video162408856_456239566'}

        bad_words = get_bad_words()

        # if not self.event.sender.check_role(Role.ADMIN):
        min_index_bad = len(clear_msg)
        max_index_bad = -1
        for word in bad_words:
            ind = clear_msg.lower().find(word)
            if ind != -1:
        ...
        min_index_bad = clear_msg.rfind(' ', 0, min_index_bad)
        if min_index_bad == -1:
            min_index_bad = clear_msg.rfind(',', 0, min_index_bad)
            if min_index_bad == -1:
                min_index_bad = clear_msg.rfind('.', 0, min_index_bad)
                if min_index_bad == -1:
                    min_index_bad = clear_msg.find('/')
        min_index_bad += 1
        if max_index_bad != -1:
            len_bad = clear_msg.find(',', max_index_bad)
            if len_bad == -1:
                len_bad = clear_msg.find(' ', max_index_bad)
                if len_bad == -1:
                    len_bad = clear_msg.find('?', max_index_bad)
                    if len_bad == -1:
                        len_bad = len(clear_msg)
            ...
            if self.event.sender.gender == '1':
                ...
                msg_self = "сам"
            messages.append(f"{name}, может ты {msg_self} {clear_msg[min_index_bad: len_bad]}?")
            return messages
