...
class GPTCommand(ABC, Command):
    ...
    def _get_stat_for_user(self, profile: Profile) -> Tuple[str | None, io.BytesIO | None]:
        """
    ...
    def _get_data_for_statistics_plot(self, profile):
        one_month_ago = timezone.now() - datetime.timedelta(days=30)

        usage_stats = GPTUsage.objects.filter(
            author=profile,
    ...
    @staticmethod
    def _get_statistics_plot(stats_dict) -> io.BytesIO:
        ...
        plt.title('Использование GPT за последние 30 дней', fontsize=16)
