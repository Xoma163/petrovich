from typing import Any, Callable, Protocol

from apps.bot.consts import RoleEnum
from apps.bot.core.bot.bot import Bot
from apps.bot.core.event.event import Event
from apps.bot.core.messages.response_message import ResponseMessage
from apps.commands.help_text import HelpText


class CommandProtocol(Protocol):
    name: str
    names: list
    help_text: HelpText | None
    priority: int
    abstract: bool

    # Проверки и ограничения
    access: RoleEnum
    pm: bool
    conversation: bool
    fwd: bool
    args: int
    args_or_fwd: bool
    int_args: list
    platforms: list
    attachments: list
    mentioned: bool
    non_mentioned: bool

    def __init__(self, bot: Bot | None = None, event: Event | None = None): ...  # noqa

    bot: Bot
    event: Event

    def accept(self, event: Event) -> bool: ...

    def check_and_start(self, bot: Bot, event: Event) -> ResponseMessage: ...

    def checks(self) -> None: ...

    def start(self) -> ResponseMessage | None: ...

    def check_sender(self, role: RoleEnum) -> None: ...

    def check_args(self, args: int | None = None) -> bool: ...

    def check_args_or_fwd(self, args: int | None = None) -> bool: ...

    @staticmethod
    def check_number_arg_range(
        arg: Any, _min: float = -float("inf"), _max: float = float("inf"), banned_list: list[Any] | None = None
    ) -> None: ...

    def parse_int(self) -> None: ...

    def check_pm(self) -> bool: ...

    def check_conversation(self) -> bool: ...

    def check_fwd(self) -> bool: ...

    def check_platforms(self) -> bool: ...

    def check_attachments(self) -> None: ...

    def check_mentioned(self) -> None: ...

    def check_non_mentioned(self) -> bool: ...

    def handle_menu(self, menu: list, arg: str) -> Callable: ...

    def _get_help_button_keyboard(self) -> None: ...

    def __eq__(self, other) -> bool: ...

    def __hash__(self) -> int: ...

    def __str__(self) -> str: ...


class AcceptExtraProtocol(Protocol):
    @staticmethod
    def accept_extra(event) -> bool: ...
