...
class Command(BaseCommand):
    ...
    def __init__(self):
        ...
        # self.now = datetime.datetime(2020, 9, 7, 18, 35, 1)
    ...
    def handle(self, *args, **kwargs):
        lesson_number = str(self.get_lesson_number(self.now))

        lesson = self.schedule.get(self.now_weeknumber, {}).get(self.now_weekday, {}).get(lesson_number)
        if lesson:
